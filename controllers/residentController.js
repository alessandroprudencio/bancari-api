...
const createResident = async (req, res) => {
    ...
    if (!name || !email || !address || !phone || !number_address) return status(400).send({ message: 'Por favor preencha todos os campos' })

    if (email && !validator.isEmail(email)) return status(400).send({ message: 'E-mail inválido' })

    try {
        await (knex('residents').insert(req.body))
        res.json({ message: "Morador cadastrado com sucesso!" })
    } catch (error) {
        if (error.code == "ER_DUP_ENTRY") status(400).send({ message: 'Email já cadastrado!' })
        status(500).send({ message: error })
    }
}
...
const deleteResident = async (req, res) => {
    ...
    try {
        ...
        if (!resident.length) return status(404).send({ message: "Usuário não encontrado" })

        let token = req.headers.authorization.split(" ")[1]
        if (jwt.decode(token).admin === 0) return status(401).send({ message: "Usuário não tem permissões para exclusão" })

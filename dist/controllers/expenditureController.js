...
const getByIdExpenditure = async (req, res) => {
  try {
    res.send((await knex('expenditures').select('id', 'value', 'updated_at').orderBy('updated_at', 'desc').where({
      id: req.params.id
    })));
  } catch (err) {
    res.status(500).send({
...
const createExpenditure = async (req, res) => {
  ...
  if (!value || !name) return res.status(400).send({
...
const deleteExpenditure = async (req, res) => {
  try {
    let resident = await knex.select('id').from('expenditures').where({
      id: req.params.id
    });
...
exports.getExpenditure = getExpenditure;
exports.getByIdExpenditure = getByIdExpenditure;

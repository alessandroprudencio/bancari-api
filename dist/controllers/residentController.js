...
const getByIdResident = async (req, res) => {
  try {
    res.send((await knex.select('id', 'name', 'email', 'address', 'phone', 'number_address').from('residents').where({
      id: req.params.id
    })));
  } catch (err) {
    res.status(500).send({
...
const createResident = async (req, res) => {
  ...
  if (!name || !email || !address || !phone || !number_address) return res.status(400).send({
...
const deleteResident = async (req, res) => {
  try {
    let resident = await knex.select('id').from('residents').where({
      id: req.params.id
    });
...
exports.getByIdResident = getByIdResident;

...
const getByIdRealty = async (req, res) => {
  try {
    res.send((await knex('realtys').leftJoin('residents', 'realtys.owner_id', 'residents.id').select('residents.name', 'realtys.address', 'realtys.id', 'realtys.number_address').where({
      id: req.params.id
    })));
  } catch (err) {
    res.status(500).send({
...
const createRealty = async (req, res) => {
  ...
  if (!address || !number_address) return res.status(400).send({
...
const deleteRealty = async (req, res) => {
  try {
    let resident = await knex.select('id').from('realtys').where({
      id: req.params.id
    });
...
exports.getByIdRealty = getByIdRealty;
exports.createRealty = createRealty;
exports.updateRealty = updateRealty;
exports.deleteRealty = deleteRealty;
//# sourceMappingURL=realtyController.js.map